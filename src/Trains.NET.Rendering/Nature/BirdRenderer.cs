...
#pragma warning restore IDE1006 // Naming Styles
        ...
        public void Render(ICanvas canvas, int width, int height, IPixelMapper pixelMapper)
        {
            ...
            {
                Bird bird = _birds[i];
                if (bird.Location.X < 0 || bird.Location.X > pixelMapper.MaxGridSize ||
                    bird.Location.Y < 0 || bird.Location.Y > pixelMapper.MaxGridSize)
                {
                    _birds.RemoveAt(i);
                }
            }
            ...
            Bird SpawnBird()
            {
                ...
                Vector2 GetPoint(int side, IPixelMapper pixelMapper)
                {
                    ...
                    {
                        0 => 0,
                        1 or 3 => _random.Next(pixelMapper.MaxGridSize),
                        2 => pixelMapper.MaxGridSize,
                        4 => _random.Next(pixelMapper.MaxGridSize), // special for control points
                        _ => throw new ArgumentException("That's the trouble with random...")
                    };
                    var y = side switch
                    {
                        0 or 2 => _random.Next(pixelMapper.MaxGridSize),
                        1 => 0,
                        3 => pixelMapper.MaxGridSize,
                        4 => _random.Next(pixelMapper.MaxGridSize), // special for control points
                        _ => throw new ArgumentException("That's the trouble with random...")
                    };
